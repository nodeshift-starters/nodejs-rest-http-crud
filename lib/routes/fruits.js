...
router.post('/fruits', [parseRequestBodyToJson, validations.validateCreateUpdateRequest], (request, response) => {
...
router.put('/fruits/:id', [parseRequestBodyToJson, validations.validateCreateUpdateRequest], (request, response) => {
  const { name, stock } = request.body;
  const { id } = request.params;
...
function parseRequestBodyToJson(request, response, next) {
  try {
    request.body = JSON.parse(request.body);
  } catch (err) {
    return response.status(415).send('Invalid payload!');
  }
  next();
}
